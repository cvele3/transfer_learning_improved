...
######################################
PROCESSED_DATA_FILE = "processed_data_v2.pkl"
...
if os.path.exists(PROCESSED_DATA_FILE):
    ...
    filepath_raw = 'ToxinPeptideV2.xlsx'
...
# Spremanje svih predikcija i stvarnih oznaka kroz svih 10 foldova
all_y_test = []
all_y_pred_probs = []
...
for x, c, i in zip(bin_centers, correct_hist, incorrect_hist):
    # Skip if no peptides in this bin
    if c == 0 and i == 0:
        continue
...
model.save('toxicityModel25peptide_baseline_v2.h5')
...
excel_save = f"metrics_peptide_v2_baseline_k{k}_{layer_sizes}_with_{filter1}-{filter2}-{filter3}.xlsx"
