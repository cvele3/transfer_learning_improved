...
# Convert graphs to a NumPy array (if not already done) and labels to a pandas Series.
graphs = np.array(graphs)
graph_labels = pd.Series(labels)

# Separate indices by molecule type.
peptide_indices = [i for i, t in enumerate(types) if t == "peptid"]
small_molecule_indices = [i for i, t in enumerate(types) if t == "small"]

# Extract only the peptide subset for cross-validation splitting.
peptide_graphs = graphs[peptide_indices]
peptide_labels = graph_labels.iloc[peptide_indices]

# Set up a stratified K-Fold split on the peptide subset.
skf = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
histories = []

for fold, (pep_train_idx, pep_test_idx) in enumerate(skf.split(peptide_graphs, peptide_labels)):
    print(f"Processing Fold {fold + 1}...")
    
    # Map the peptide indices from the peptide subset back to the original indices.
    pep_train_indices = np.array(peptide_indices)[pep_train_idx]
    pep_test_indices = np.array(peptide_indices)[pep_test_idx]
    
    # For this fold, the test set is exclusively the selected peptides.
    X_test = graphs[pep_test_indices]
    y_test = graph_labels.iloc[pep_test_indices]
    
    # The training set is the union of:
    #   - all small molecules (all indices in small_molecule_indices), and
    #   - the peptides not selected for testing in this fold.
    fold_train_indices = np.concatenate((small_molecule_indices, pep_train_indices))
    X_train_val = graphs[fold_train_indices]
    y_train_val = graph_labels.iloc[fold_train_indices]
    
    # (Optional) Further split training data into training and validation sets.
    X_train, X_val, y_train, y_val = train_test_split(
        X_train_val, y_train_val, test_size=0.2, random_state=42, stratify=y_train_val
    )
    
    # Create a PaddedGraphGenerator based on the entire set of graphs.
    ...
    history = model.fit(
        ...
        shuffle=True,
        callbacks=[callback]  # e.g., early stopping callback
    )
    histories.append(history)
    
    # Predict and evaluate on the test set (which contains only peptides).
...
model.save('combinedModel25peptidebetter.h5')
...
filename = f"metrics_peptidebetter_test_k{k}_layers_{layer_sizes}_filters_{filter1}-{filter2}-{filter3}.xlsx"
